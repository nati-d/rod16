...
async function uploadToCloudinary(
	...
	try {
		const localPath = path.join(process.cwd(), "public", "events", imagePath);
...
// Main function
async function main() {
	console.log("🚀 Starting Events Images Upload Process...\n");
	...
	try {
		// Step 1: Delete the old events folder
		const oldFolderPath = "rod16-photography/events";
		await deleteFolder(oldFolderPath);
		console.log("");

		// Step 2: Get all images from public/events folder
		const eventsFolder = path.join(process.cwd(), "public", "events");
		
		if (!fs.existsSync(eventsFolder)) {
			console.error(`❌ Error: Folder not found: ${eventsFolder}`);
			process.exit(1);
		}

		// Get all image files
		const allFiles = fs.readdirSync(eventsFolder)
		...
		console.log(`📸 Found ${files.length} images in public/events folder\n`);
		...
		// Step 3: Upload all images to events folder
		console.log(`📤 Uploading ${files.length} images to Cloudinary (events folder)...\n`);
		...
		// Generate the new events array (filter out empty strings from failed uploads)
		const validResults = results.filter(url => url && url.length > 0);
		const newEventsArray = `// Events portfolio images
export const events = [
${validResults.map((url) => `\t"${url}",`).join("\n")}
];`;
		
		// Replace the events array in the file
		// Match the events export section (from "// Events" comment to the closing bracket and semicolon)
		const eventsRegex = /\/\/\s*Events[\s\S]*?export const events = \[[\s\S]*?\];/;
		
		if (eventsRegex.test(constantsContent)) {
			const updatedContent = constantsContent.replace(eventsRegex, newEventsArray);
			fs.writeFileSync(constantsPath, updatedContent, "utf-8");
			console.log(`✅ Updated constants/index.ts with ${validResults.length} events image URLs`);
		} else {
			console.error(`❌ Error: Could not find events array in constants/index.ts`);
			...
			fs.writeFileSync(constantsPath, updatedContent, "utf-8");
			console.log(`✅ Appended new events array to constants/index.ts`);
		}
