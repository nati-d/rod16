...
async function uploadToCloudinary(
	...
	try {
		const localPath = path.join(process.cwd(), "public", "baptism", imagePath);
...
// Main function
async function main() {
	console.log("🚀 Starting Baptism Images Upload Process...\n");
	...
	try {
		// Step 1: Check and optionally delete existing baptism folder (if it exists)
		const oldFolderPath = "rod16-photography/baptism";
		await deleteFolderIfExists(oldFolderPath);
		console.log("");

		// Step 2: Get all images from public/baptism folder
		const baptismFolder = path.join(process.cwd(), "public", "baptism");
		
		if (!fs.existsSync(baptismFolder)) {
			console.error(`❌ Error: Folder not found: ${baptismFolder}`);
			process.exit(1);
		}

		// Get all image files
		const allFiles = fs.readdirSync(baptismFolder)
		...
		console.log(`📸 Found ${files.length} images in public/baptism folder\n`);
		...
		// Step 3: Upload all images to baptism folder
		console.log(`📤 Uploading ${files.length} images to Cloudinary (baptism folder)...\n`);
		
		const results: string[] = [];
		const folderName = "baptism";
		...
		// Step 4: Update constants/index.ts file - append new baptism array
		...
		// Generate the new baptism array (filter out empty strings from failed uploads)
		const validResults = results.filter(url => url && url.length > 0);
		const newBaptismArray = `// Baptism portfolio images
export const baptism = [
${validResults.map((url) => `\t"${url}",`).join("\n")}
];`;
		
		// Check if baptism array already exists
		const baptismRegex = /\/\/\s*Baptism[\s\S]*?export const baptism = \[[\s\S]*?\];/;
		
		if (baptismRegex.test(constantsContent)) {
			// Replace existing baptism array
			const updatedContent = constantsContent.replace(baptismRegex, newBaptismArray);
			fs.writeFileSync(constantsPath, updatedContent, "utf-8");
			console.log(`✅ Updated constants/index.ts with ${validResults.length} baptism image URLs`);
		} else {
			// Append new baptism array to the end of the file
			const updatedContent = constantsContent.trimEnd() + "\n\n" + newBaptismArray + "\n";
			fs.writeFileSync(constantsPath, updatedContent, "utf-8");
			console.log(`✅ Added new baptism array to constants/index.ts with ${validResults.length} image URLs`);
		}
