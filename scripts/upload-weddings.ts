...
async function uploadToCloudinary(
	...
	try {
		...
		const uploadOptions = {
			...
			timeout: 120000, // 120 seconds timeout (2 minutes) for large compressed images
		};
...
async function main() {
	...
	try {
		...
		// Generate the new weeding array
...
${results.map((url) => `\t"${url}",`).join("\n")}
		...
		if (weedingRegex.test(constantsContent)) {
			...
			console.log(`✅ Updated constants/index.ts with ${results.length} wedding image URLs`);
		} else {
		...
		console.log(`\n📝 ${results.length} images successfully uploaded and constants file updated!`);
